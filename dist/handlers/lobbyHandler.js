...
exports.handleLobby = handleLobby;
const uuid_1 = require("uuid");
const logger_1 = require("../utils/logger");
// In-memory storage for lobbies
const lobbies = new Map();
async function handleLobby(ws, type, data, client, clients) {
    try {
        switch (type) {
            case 'create_lobby':
...
async function createLobby(ws, data, client, clients) {
    ...
    logger_1.logger.info(`Lobby ${lobbyId} created by ${hostId}`);
}
async function joinLobby(ws, data, client, clients) {
    ...
    logger_1.logger.info(`Player ${playerId} joined lobby ${lobbyId}`);
}
async function leaveLobby(ws, data, client, clients) {
    ...
    if (playerId === lobby.hostId) {
        if (lobby.players.length > 0) {
            ...
            logger_1.logger.info(`Lobby ${lobbyId} deleted (no players left)`);
            return;
        }
    }
    // Update lobby status
    ...
    logger_1.logger.info(`Player ${playerId} left lobby ${lobbyId}`);
}
async function setPlayerReady(ws, data, client, clients) {
    ...
    logger_1.logger.info(`Player ${playerId} ready status updated to ${player.isReady} in lobby ${lobbyId}`);
}
async function startGame(ws, data, client, clients) {
    ...
    logger_1.logger.info(`Game started for lobby ${lobbyId}, session ${sessionId}`);
}
function notifyLobbyUpdate(lobbyId, clients) {
    if (!lobbies.has(lobbyId))
        return;
    const lobby = lobbies.get(lobbyId);
